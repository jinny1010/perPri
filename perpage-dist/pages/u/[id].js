...
export default function UserPage() {
  ...
  const [sitePasswordEnabled, setSitePasswordEnabled] = useState(true);
  const [sitePassword, setSitePassword] = useState('0000');
  const [galleryPassword, setGalleryPassword] = useState('0406');
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (myData) {
        ...
        if (parsed.encryptedId === id) {
          setUserData(parsed);
          setSub(parsed.characterName);
          
          // 설정 로드
          const savedEnabled = localStorage.getItem(`sitePasswordEnabled_${id}`);
          const savedSitePass = localStorage.getItem(`sitePassword_${id}`);
          const savedGalleryPass = localStorage.getItem(`galleryPassword_${id}`);
          
          if (savedEnabled !== null) setSitePasswordEnabled(savedEnabled === 'true');
          if (savedSitePass) setSitePassword(savedSitePass);
          if (savedGalleryPass) setGalleryPassword(savedGalleryPass);
          
          // 비밀번호 체크
          if (savedEnabled === 'false') {
            setSiteUnlocked(true);
          } else {
            setShowSitePassword(true);
          }
  ...
  const fetchFolderInfo = async () => {
    try {
      ...
      const data = await res.json();
      console.log('fetchFolderInfo response:', data);
      if (!res.ok) throw new Error(data.message);
      const folders = data.folders || [];
      console.log('Looking for sub:', sub, 'in folders:', folders.map(f => f.name));
      const folder = folders.find(f => f.name === sub);
      console.log('Found folder:', folder);
      if (folder) {
        setFolderInfo(folder);
        setFolderIndex(folders.indexOf(folder) + 1);
      }
    } catch (err) {
      console.error('fetchFolderInfo error:', err);
    }
  };
  ...
  const fetchPosts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`/api/posts?sub=${encodeURIComponent(sub)}`, { headers: getHeaders() });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
      setPosts(data.posts || []);
    } catch (err) {
  ...
  const fetchBookmarks = async () => {
    ...
    try {
      const res = await fetch(`/api/bookmarks?sub=${encodeURIComponent(sub)}`, { headers: getHeaders() });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  const fetchThemes = async () => {
    try {
      const res = await fetch(`/api/themes?sub=${encodeURIComponent(sub)}`, { headers: getHeaders() });
      const data = await res.json();
      if (res.ok) {
  ...
  // 사이트 비밀번호 확인
  const handleSitePasswordSubmit = () => {
    if (sitePasswordInput === sitePassword) {
      setSiteUnlocked(true);
      setShowSitePassword(false);
      setSitePasswordInput('');
      setSitePasswordError('');
    } else {
      setSitePasswordError('비밀번호가 틀렸습니다');
    }
  };
  ...
  const handleApiKeySubmit = async () => {
    ...
    try {
      ...
      // 캐릭터 이름 입력 받기
      const charName = prompt('캐릭터 이름을 입력하세요 (예: Killian Vane)');
      if (!charName) return;
  ...
  const handleSaveSettings = () => {
    if (newSitePassword) {
      ...
      localStorage.setItem(`sitePassword_${id}`, newSitePassword);
    }
    if (newGalleryPassword) {
      setGalleryPassword(newGalleryPassword);
      localStorage.setItem(`galleryPassword_${id}`, newGalleryPassword);
    }
  ...
  const openPost = async (post) => {
    ...
    try {
      const res = await fetch(`/api/content?pageId=${post.id}`, { headers: getHeaders() });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  const handleDeletePost = async () => {
    ...
    try {
      const res = await fetch(`/api/delete?pageId=${deleteTarget.id}`, { method: 'DELETE', headers: getHeaders() });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message);
  ...
  if (killed) {
    return (
      <>
        ...
        <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a1a2e 0%, #0f0f1a 100%)', color: 'white', textAlign: 'center' }}>
          <div style={{ fontSize: 80 }}>🚫</div>
          <h2 style={{ fontSize: 28, marginBottom: 10 }}>서비스 종료</h2>
          <p style={{ color: '#666' }}>이 사이트는 더 이상 이용할 수 없습니다.</p>
        </div>
      </>
    );
  }
  ...
  if (needsApiKey) {
    return (
      <>
        <Head><title>API 키 입력</title></Head>
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: 'linear-gradient(135deg, #1a1a2e 0%, #16213e 100%)', padding: 20 }}>
          <div style={{ background: 'white', borderRadius: 20, padding: 40, textAlign: 'center', maxWidth: 400, width: '100%' }}>
            <div style={{ fontSize: 48, marginBottom: 10 }}>🔑</div>
            <h2 style={{ fontSize: 20, marginBottom: 5 }}>다른 기기에서 접속</h2>
            <p style={{ color: '#666', marginBottom: 20, fontSize: 14 }}>Notion API 키를 입력하세요</p>
            <input type="password" value={apiKeyInput} onChange={(e) => setApiKeyInput(e.target.value)} placeholder="secret_xxxxx..." style={{ width: '100%', padding: 14, border: '2px solid #eee', borderRadius: 10, fontSize: 14, marginBottom: 15, boxSizing: 'border-box' }} />
            <button onClick={handleApiKeySubmit} style={{ width: '100%', padding: 14, background: '#1a1a2e', color: 'white', border: 'none', borderRadius: 10, fontSize: 16, cursor: 'pointer' }}>확인</button>
          </div>
        </div>
      </>
    );
        ...
        <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: `linear-gradient(135deg, ${savedColor} 0%, #1a1a2e 100%)`, padding: 20 }}>
          <div style={{ background: 'rgba(255,255,255,0.95)', borderRadius: 20, padding: 40, textAlign: 'center', maxWidth: 320, width: '100%', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <input 
              ...
              onChange={(e) => { setSitePasswordInput(e.target.value); setSitePasswordError(''); }}
              onKeyDown={(e) => e.key === 'Enter' && handleSitePasswordSubmit()}
              placeholder="비밀번호"
              autoFocus
  ...
  return (
    <>
      ...
      <button 
        ...
        style={{ position: 'fixed', top: 15, right: 15, width: 40, height: 40, borderRadius: '50%', background: 'rgba(255,255,255,0.9)', border: 'none', fontSize: 20, cursor: 'pointer', zIndex: 1000 }}
      >
        ⚙️
      </button>
        ...
        <div className="modal-overlay" onClick={() => setShowSettings(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-buttons">
              ...
              <button className="btn-submit" onClick={handleSaveSettings}>저장</button>
            </div>
          </div>
        </div>
      )}
